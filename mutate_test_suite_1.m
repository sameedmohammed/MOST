function mutationChildren = mutate_test_suite_1(parents ,options,NVARS, ...
FitnessFcn, state, thisScore,thisPopulation,mutationRate)
disp ( 'In mutation ...')
filename = 'mutate.xlsx';
...
mutated = zeros(length(parents));
numMutated = 0;
for i=1:length(parents)
            if (randi([1,100])==1)
                mutated(i) = 1;
                mutationChildren{i} =  mutationChildren{i};
...
scores = zeros (numMutated,4);
index = 1;
for j = 1 : length(parents)
            if(mutated(j) == 1)
                scores(index,:)= multiobjective_fitness_4_3(mutationChildren{j});
                index = index + 1;
            end
end
xlswrite(filename,scores);
