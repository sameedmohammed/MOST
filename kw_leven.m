    ...
    if(delim(i)+1 == delim(i+1))
        ...
        for j = 1 : (length(delim)-1)
            if(delim(j)+1 ~= delim(j+1))
                num = num + 1;
                if(levenDiv (x((delim(i)+1):(delim(i+1)-1)),x((delim(j)+1):(delim(j+1)-1)))> max(1,i))
                    max(1,i) =  levenDiv (x((delim(i)+1):(delim(i+1)-1)),x((delim(j)+1):(delim(j+1)-1)));
                end
            end
        end
    end
