function mutationChildren = mutate_test_suite_2(parents ,options,NVARS, ...
FitnessFcn, state, thisScore,thisPopulation,mutationRate)
%{  
creates a cell array (mutationChildren) containing chromosomes with a
certain length (NVARS) resulting from the mutation of the selected
chromosomes (parents) from the previous generetion (thisPopulation)
%}
...
for i=1:length(parents)
    parentFixed = thisPopulation{parents(i)}; % Normally thisPopulation(parents(i),:)
    parent = cast_to_variable_length( parentFixed);
    
    % store locations of beginnings and ends of genes (marked by zeros)
    delim1 = find (parent == 0);
    delim = [0,delim1];
    ...
    if(len <=1)
        ...
        switch random
            ...
            case 2
                %adding a gene
                toBeAdded = randi ([1,maxOutgoing],1,randi([1,10]));
                if(parent(end) == 0)
        ...
        childFixed = zeros(1,NVARS);
        for l = 1: length(childFixed)
            if( l<= length(child))
                childFixed(l) = child(l);
            else
                childFixed(l) = -1;
            end
        end
        
        mutationChildren{i} = childFixed; % Normally mutationChildren(i,:)
    end
    
end
mutationChildren = fixpopulation(mutationChildren,NVARS);
disp('out of mutation')
disp(length(mutationChildren))
end
